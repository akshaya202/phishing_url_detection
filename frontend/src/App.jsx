...
import { Activity, AlertTriangle, BarChart3, CheckCircle2, Lock, Shield, ShieldAlert, ShieldCheck, Settings, UserCircle2, ChartNoAxesCombined } from 'lucide-react';
...
const getStatusBadge = (prediction) => {
  if (prediction === 'Phishing') return 'bg-rose-500/15 text-rose-300 border border-rose-500/30';
  if (prediction === 'Suspicious') return 'bg-amber-500/15 text-amber-300 border border-amber-500/30';
  return 'bg-emerald-500/15 text-emerald-300 border border-emerald-500/30';
};

const getStatusLabel = (prediction) => {
  if (prediction === 'Phishing') return 'PHISHING';
...
function App() {
  ...
  const handleReport = async () => {
    if (!reportUrl) return;
    try {
      await axios.post(`${API_BASE}/api/reports`, { url: reportUrl }, buildAuthHeader(token));
      alert('Suspicious URL reported successfully. It will be reviewed by an administrator before training is updated.');
      setReportUrl('');
      fetchStats();
    } catch (error) {
  ...
  const flaggedPhishing = history.filter((h) => h.prediction === 'Phishing').length;
  const verifiedSafe = history.filter((h) => h.prediction === 'Legitimate').length;

  const filteredHistory = useMemo(() => {
    if (filterStatus === 'All Status') return history;
    if (filterStatus === 'Phishing') return history.filter((h) => h.prediction === 'Phishing');
    if (filterStatus === 'Suspicious') return history.filter((h) => h.prediction === 'Suspicious');
    if (filterStatus === 'Safe') return history.filter((h) => h.prediction === 'Legitimate');
    return history;
                  ...
                  {scanResult && (
                    <div className="mt-6 rounded-2xl border border-slate-700 bg-slate-950/80 p-5">
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          <p className="text-xs uppercase tracking-[0.2em] text-slate-400">Latest Scan Result</p>
                          <h3 className="mt-2 text-2xl font-bold text-white">{scanResult.prediction}</h3>
                          <p className="mt-1 text-sm text-slate-400">{scanResult.url}</p>
                        </div>
                        <span className={`badge ${statusColors[scanResult.prediction] || 'bg-slate-700 text-slate-200'}`}>
                      ...
                      <div className="mt-6 grid gap-4 md:grid-cols-3">
                        <div className="rounded-xl border border-slate-700 bg-slate-900 p-4">
                          ...
                          <p className="mt-2 text-2xl font-bold text-cyan-300">{Number(scanResult.confidence * 100).toFixed(1)}%</p>
                        </div>
                        <div className="rounded-xl border border-slate-700 bg-slate-900 p-4">
                          <p className="text-xs text-slate-400">Risk Score</p>
                          <p className="mt-2 text-2xl font-bold text-amber-300">{Number(scanResult.risk * 100).toFixed(1)}%</p>
                        </div>
                        <div className="rounded-xl border border-slate-700 bg-slate-900 p-4">
                          <p className="text-xs text-slate-400">Model Version</p>
                          <p className="mt-2 text-xl font-bold text-emerald-300">{scanResult.model_version || stats.modelVersion}</p>
                        </div>
                      </div>
                ...
                <div className="card p-5">
                  ...
                  {user ? (
                    <div className="space-y-3">
                      ...
                      <button className="btn-secondary w-full" onClick={() => { setToken(''); setUser(null); }}>Logout</button>
                    </div>
                  ) : (
